...
if __name__ == '__main__':
    ...
    # 读取数据
    df = pd.read_json(f"data/{args.data_name}/filtered_statement_result.json")
    ...
    for row, output in zip(data_lst, outputs):
        ...
        result_lst.append({
            ...
            "statement": row["statement"],
            "filter_flag": row["filter_flag"],
            "full_source": output_text,
            "source": source,
        })

    output_file_path = f"data/{args.data_name}/source_statement_result.json"
